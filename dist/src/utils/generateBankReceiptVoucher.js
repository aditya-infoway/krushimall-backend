...
export const generateBankReceiptVoucher = async (companyId, financialYearId) => {
    const prefixMaster = await prisma.profilePrefix.findFirst({
        where: {
            prefixFor: "BANK_RECEIPT",
        },
    });
    if (!prefixMaster) {
        throw new Error("Bank Receipt prefix not found");
    }
    // Selected session/FY
    ...
    const lastVoucher = await prisma.bankReceipt.findFirst({
        where: {
            companyId,
            financialYearId,
            voucherNo: {
...
//# sourceMappingURL=generateBankReceiptVoucher.js.map