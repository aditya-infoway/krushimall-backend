...
import ExcelJS from "exceljs";
// ==========================================
// SHARED HELPER: Build all ledger transactions for an account
// Used by both getLedgerDetails and exportLedgerDetails
// ==========================================
const buildLedgerTransactions = async (accountId) => {
    const [cashPayments, bankPayments, purchases, accessoriesPurchases, contras, cashReceipts, bankReceipts, leads,] = await Promise.all([
        prisma.cashPayment.findMany({
            where: {
                OR: [{ cashAccountId: accountId }, { oppAccountId: accountId }],
            },
            include: {
                cashAccount: true,
                oppAccount: true,
        ...
        prisma.bankPayment.findMany({
            where: {
                OR: [{ bankAccountId: accountId }, { oppAccountId: accountId }],
            },
            include: {
                bankAccount: true,
                oppAccount: true,
        ...
        prisma.accessoriesPurchase.findMany({
            where: {
                accountId,
            },
            include: {
        ...
        prisma.contra.findMany({
            where: {
                OR: [{ cashBankAccountId: accountId }, { oppAccountId: accountId }],
            },
            include: {
    ...
    transactions.sort((a, b) => {
        ...
        // Same transaction date: sort by record creation time
...
// ==========================================
// SHARED HELPER: Apply date range filter, opening balance,
// running balance, and search filter
// ==========================================
const applyLedgerFilters = (transactions, account, fromDate, toDate, search) => {
    // ==========================================
    ...
    return result.filter((item) => String(item.type || "")
        .toLowerCase()
        .includes(value) ||
...
// ==========================================
// GET LEDGER DETAILS (for a single account, JSON response)
// ==========================================
export const getLedgerDetails = async (req, res) => {
    try {
        ...
        const transactions = await buildLedgerTransactions(accountId);
        const filtered = applyLedgerFilters(transactions, account, fromDate ? String(fromDate) : undefined, toDate ? String(toDate) : undefined, search ? String(search) : undefined);
        return res.json({
            success: true,
            account: {
...
// ==========================================
// GET LEDGER REPORT (list of all accounts, JSON response)
// ==========================================
...
// ==========================================
// EXPORT LEDGER REPORT (list of all accounts, Excel download)
// ==========================================
export const exportLedgerReport = async (req, res) => {
    try {
        const { search } = req.query;
        const accounts = await prisma.account.findMany({
            where: search
        ...
        res.setHeader("Content-Disposition", 'attachment; filename="LedgerReport.xlsx"');
...
// ==========================================
// EXPORT LEDGER DETAILS (single account, Excel download)
// ==========================================
export const exportLedgerDetails = async (req, res) => {
    try {
        const accountId = Number(req.params.id);
        const { search, fromDate, toDate } = req.query;
        const account = await prisma.account.findUnique({
        ...
        res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        res.setHeader("Content-Disposition", 'attachment; filename="LedgerDetails.xlsx"');
        await workbook.xlsx.write(res);
        res.end();
    }
    catch (err) {
