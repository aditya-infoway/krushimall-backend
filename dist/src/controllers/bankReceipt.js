...
export const getBankReceiptById = async (req, res) => {
    try {
...
export const createBankReceipt = async (req, res) => {
    try {
        const { companyId, financialYearId, date, bankAccountId, oppAccountId, leadId, amount, narration, paymentType, chequeNo, chequeDate, chequeClearDate, } = req.body;
        const user = req.user;
        const role = user?.role?.toUpperCase();
        ...
        const voucherNo = await generateBankReceiptVoucher(Number(companyId), Number(financialYearId));
...
export const getBankReceiptVoucher = async (req, res) => {
    try {
...
export const updateBankReceipt = async (req, res) => {
    try {
        const id = Number(req.params.id);
        const { date, bankAccountId, oppAccountId, leadId, amount, narration, paymentType, chequeNo, chequeDate, chequeClearDate, } = req.body;
...
export const deleteBankReceipt = async (req, res) => {
    try {
        const id = Number(req.params.id);
...
export const exportBankReceiptExcel = async (req, res) => {
    try {
        const receipts = await prisma.bankReceipt.findMany({
            orderBy: {
                id: "desc",
        ...
        res.setHeader("Content-Disposition", 'attachment; filename="BankReceiptRegister.xlsx"');
...
export const printBankReceipt = async (req, res) => {
    try {
        const id = Number(req.params.id);
        const user = req.user;
        const role = user?.role?.toUpperCase();
        ...
        const receiver = receipt.lead?.customer || receipt.oppAccount;
...
`;
        ...
        res.setHeader("Content-Disposition", `inline; filename=receipt-${receipt.voucherNo}-${Date.now()}.pdf`);
