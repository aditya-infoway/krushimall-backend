...
function computeFollowupStage(latestFollowup) {
...
export const createWebsiteEnquiry = async (req, res) => {
    try {
        const { websiteVariantId, usedWebsiteVariantId, fullName, email, mobileNumber, interestedIn, message, } = req.body;
        // Validate Variant
        if (interestedIn === "new") {
        ...
        const enquiry = await prisma.websiteEnquiry.create({
            data: {
                websiteVariantId: interestedIn === "new" ? Number(websiteVariantId) : null,
                usedWebsiteVariantId: interestedIn === "used" ? Number(usedWebsiteVariantId) : null,
                fullName,
                email,
                mobileNumber,
                interestedIn,
...
export const getWebsiteEnquiry = async (req, res) => {
    try {
        const vendor = req.vendor;
...
//# sourceMappingURL=websiteEnquiry.js.map