...
export const createVehicleStockTransfer = async (req, res) => {
    try {
        const { companyId, financialYearId, transferDate, branchId, managerId, branchOpeningBalance, totalValue, freight, insurance, otherCharge, taxableValue, cgst, sgst, igst, grandTotal, vehicles, } = req.body;
        if (!companyId ||
            !financialYearId ||
            !transferDate ||
            !branchId ||
...
export const getVehicleStockTransferNo = async (req, res) => {
    try {
        const transferNo = await generateVehicleStockTransferNo();
...
export const getVehicleStockTransfers = async (req, res) => {
    try {
        ...
        const data = await Promise.all(transfers.map(async (item) => {
...
export const getVehicleStockTransferById = async (req, res) => {
    try {
...
export const updateVehicleStockTransfer = async (req, res) => {
    try {
        const id = Number(req.params.id);
        const { companyId, financialYearId, transferDate, branchId, managerId, branchOpeningBalance, totalValue, freight, insurance, otherCharge, taxableValue, cgst, sgst, igst, grandTotal, vehicles, } = req.body;
