...
export const createCashReceipt = async (req, res) => {
    try {
        const { companyId, financialYearId, date, cashAccountId, oppAccountId, leadId, amount, narration, } = req.body;
        if (!financialYearId) {
            res.status(400).json({
        ...
        const voucherNo = await generateCashReceiptVoucher(Number(companyId), Number(financialYearId));
...
// Update Cash Payment
// ==========================
export const updateCashReceipt = async (req, res) => {
    try {
        const id = Number(req.params.id);
        const { date, type, cashAccountId, oppAccountId, leadId, amount, narration, } = req.body;
...
export const getCashReceiptVoucher = async (req, res) => {
    try {
...
export const exportCashReceiptExcel = async (req, res) => {
    try {
        ...
        res.setHeader("Content-Disposition", 'attachment; filename="CashReceiptRegister.xlsx"');
...
export const printCashReceipt = async (req, res) => {
    try {
        const id = Number(req.params.id);
        const user = req.user;
        const role = user?.role?.toUpperCase();
