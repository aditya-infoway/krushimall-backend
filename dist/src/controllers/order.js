...
export const createOrder = async (req, res) => {
    try {
        ...
        const { companyId, financialYearId, leadId, vehicleCharges = {}, allotment = {}, hypothecation = {}, exchange = {}, payment = {}, broker = {}, delivery = {}, selectedAccessories = [], } = req.body;
        // ======================================
        // BASIC VALIDATION
        // ======================================
        if (!companyId) {
        ...
        if (hypothecation.paymentStatus === "received" &&
            ...
            return res.status(400).json({
                success: false,
                message: "Cash amount and bank amount total must be equal to margin amount",
            });
        }
        ...
        const order = await prisma.$transaction(async (tx) => {
            const createdOrder = await tx.order.create({
                data: {
                    ...
                    rtoRegistrationCharge: toNumber(vehicleCharges.rtoRegistrationCharge ??
                    ...
                    processingCharge: toNumber(hypothecation.processingCharge ?? hypothecation.apronCharge),
                    ...
                    exchangeInsurance: exchange.insurance != null ? String(exchange.insurance) : null,
            ...
            if (selectedAccessories.length > 0) {
                await tx.orderAccessory.createMany({
                    data: selectedAccessories.map((item) => ({
            ...
            if (hypothecation.paymentStatus === "received" &&
                ...
                await tx.bankReceipt.create({
                    data: {
                        ...
                        chequeNo: hypothecation.paymentMode === "CHEQUE"
                            ? hypothecation.chequeNo || null
                            : null,
                        chequeDate: hypothecation.paymentMode === "CHEQUE"
                            ? toOptionalDate(hypothecation.chequeDate)
                            : null,
                        chequeClearDate: hypothecation.paymentMode === "CHEQUE"
        ...
        if (error?.code === "P2003") {
            return res.status(400).json({
                success: false,
                message: "Invalid company, financial year, lead, finance, cash account, or bank account",
            });
        }
...
// GET /api/orders
// ==========================================
export const saveAccessoriesAllotment = async (req, res) => {
    try {
...
// GET /api/orders/vehicle-verify-accessories
// ==========================================
export const getVehicleVerifyAccessories = async (req, res) => {
    try {
        ...
        const data = orders.map((order, index) => {
            ...
            const completedAccessories = order.orderAccessories.filter((item) => item.status === "Completed");
            const pendingAccessories = order.orderAccessories.filter((item) => item.status === "Pending");
            return {
                srNo: index + 1,
                id: order.id,
...
// ==========================================
export const verifyAccessoryItem = async (req, res) => {
    try {
...
// ==========================================
export const getVehicleInchargeList = async (req, res) => {
    try {
        const user = req.user;
        const whereClause = {};
...
// ==========================================
export const getAccessoriesAllotList = async (req, res) => {
    try {
        const user = req.user;
        const whereClause = {};
        if (user?.role?.toUpperCase() === "BRANCH") {
        ...
        for (const order of orders) {
            ...
            const completedAccessories = accessories.filter((item) => item.status === "Completed").length;
            const status = accessories.length > 0 && completedAccessories === accessories.length
                ? "completed"
                : "pending";
            data.push({
                id: order.id,
...
export const getAccessoriesAllotDetails = async (req, res) => {
    try {
        const orderId = Number(req.params.id);
...
export const allotAccessoryStock = async (req, res) => {
    try {
...
// ==========================================
export const getOrderByLeadId = async (req, res) => {
    try {
...
export const completeVehicleIncharge = async (req, res) => {
    try {
        const id = Number(req.params.id);
...
export const printDeliveryChallan = async (req, res) => {
    try {
        const leadId = Number(req.params.leadId);
        if (!Number.isInteger(leadId)) {
        ...
        if (!order) {
            res.status(404);
            return res.send("<h2 style='font-family:sans-serif;text-align:center;margin-top:60px;'>Order not created yet for this lead. Please create the order first.</h2>");
        }
        ...
        const html = generateDeliveryChallanHtml({
            ...
            addressLine1: order.company?.addressLine1 ?? "",
            mobileNumber: order.company?.mobileNumber ??
                order.company?.mobileNumber ??
                "",
        ...
        res.status(500);
        return res.send("<h2 style='font-family:sans-serif;text-align:center;margin-top:60px;'>Failed to generate delivery challan.</h2>");
    }
};
// ==========================================
// DELETE ORDER
