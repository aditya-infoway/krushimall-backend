...
export const createShowroomVariant = async (req, res) => {
    try {
        const { modelId, variantName, purPrice, purTaxPercent, exShowroomPrice, exShowroomTaxPercent, insurance, insuranceTaxPercent, rtoCharge, rtoTaxType, rtoTaxPercent, salesPrice, status, accessories = [], } = req.body;
        ...
        const variant = await prisma.showroomVariant.create({
            data: {
                modelId: Number(modelId),
                variantName,
...
export const getShowroomVariantById = async (req, res) => {
    try {
        const variant = await prisma.showroomVariant.findUnique({
            where: {
...
export const updateShowroomVariant = async (req, res) => {
    try {
        ...
        const { accessories = [], ...rest } = req.body;
        const existing = await prisma.showroomVariant.findFirst({
            where: {
                id: {
                    not: id,
        ...
        const variant = await prisma.showroomVariant.update({
            where: { id },
            data: {
                ...rest,
                modelId: Number(rest.modelId),
                purPrice: Number(rest.purPrice),
                purTaxPercent: Number(rest.purTaxPercent),
                exShowroomPrice: Number(rest.exShowroomPrice),
                exShowroomTaxPercent: Number(rest.exShowroomTaxPercent),
                insurance: Number(rest.insurance),
                insuranceTaxPercent: Number(rest.insuranceTaxPercent),
                rtoCharge: Number(rest.rtoCharge),
                rtoTaxPercent: Number(rest.rtoTaxPercent),
