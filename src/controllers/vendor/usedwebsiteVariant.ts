...
// Create
export const createUsedWebsiteVariant = async (req: Request, res: Response) => {
  try {
...
// List
export const getUsedWebsiteVariants = async (req: Request, res: Response) => {
  try {
    const vendorAuth = (req as any).vendor;
  ...
  try {
    ...
    if (similarProducts.length < 4) {
      const extraProducts = await prisma.usedWebsiteVariant.findMany({
        where: {
          id: {
            notIn: [current.id, ...similarProducts.map((item) => item.id)],
          },
          status: "ACTIVE",
          categoryId: current.categoryId,
        },
...
// Update
export const updateUsedWebsiteVariant = async (req: Request, res: Response) => {
  try {
    const vendorAuth = (req as any).vendor;
...
// Submit
export const submitUsedWebsiteVariant = async (req: Request, res: Response) => {
  try {
    const vendorAuth = (req as any).vendor;
    ...
    const updatedVariant = await prisma.usedWebsiteVariant.update({
      ...
      data: {
        agreed: true,
        isCompleted: true,
        status: "ACTIVE", // or "SUBMITTED" if that's your status
...
// Delete
export const deleteUsedWebsiteVariant = async (req: Request, res: Response) => {
  try {
    const vendorAuth = (req as any).vendor;
